...
_RULES = [
    ("Payments & Credits", r"\b(payment received|bbps payment|cc payment|bppy|mb payment|neft|imps|upi.*received|received|reversal|refund|cashback|credit)\b"),
    ("Fees, Taxes & Interest", r"\b(igst|cgst|sgst|gst|markup|finance charge|interest|late payment|surcharge|fee|annual charge|tax)\b"),
    ("Groceries", r"\b(zepto|blinkit|blink commerce|bigbasket|big basket|grofers|dmart|d-mart|reliance fresh|fresh|grocery|instamart|jiomart|amazon in grocery|freshpik|licious|amrit)\b"),
    ...
    ("Travel", r"\b(makemytrip|mmt|smartbuy|goibibo|cleartrip|ixigo|ease ?my ?trip|interglobe|flight|emt|air ?india|indigo|vistara|irctc|uber|ola|grab|rapido|bts|metro|airport lounge|loungeone|lounge|dtac|akbar|yatra)\b"),
    ("Shopping", r"\b(amazon|flipkart|myntra|ajio|nykaa|zara|westside|adidas|nike|uniqlo|hm\b|h&m|store|mall|shop|sportskart|jd sports|burberry|calvin klein|sephora|eveandboy|boots|sapphire|trend|gift shop|premium outle|decathlon|reliance digital|croma)\b"),
...
_PAYMENT_RE = re.compile(
    r"\b(payment received|bbps|bppy|cc payment|mb payment|neft|imps|rtgs|upi.*received"
